          ...
          {appointments.map((item,index)=>(
           ...
           <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
                {!item.cancelled && !item.isCompleted && <button onClick={() => appointmentPayment(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-300'>Pay Online</button> }
